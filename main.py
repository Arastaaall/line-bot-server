...
async def process_image_event(event):
    ...
    try:
        ...
        # 7秒以内に完了 ➔ Replyで返信
        await asyncio.to_thread(send_reply_sync, reply_token, result_text)
        sheets.save_to_sheet(user_id, result_text)

    except asyncio.TimeoutError:
        # 7秒超え ➔ Replyで遅延通知を返し、後でPush送信
        await asyncio.to_thread(
            send_reply_sync, 
            reply_token, 
            "⏳ 解析に少し時間がかかっております。完了次第、Push通知でお知らせします！"
        )
        ...
        # Push通知で送信
        await asyncio.to_thread(send_push_sync, user_id, result_text)
        sheets.save_to_sheet(user_id, result_text)
